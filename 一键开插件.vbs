...
Sub ProcessSubfolder(ByVal subfolder, ByVal openPlugin)
    ...
    For Each objSubfolder In objFolder.Subfolders
        ...
        If LCase(objSubfolder.Name) = "content" Then
			If openPlugin Then
				' 复制脚本当前目录下的 "CopyPlugin" 中的所有文件到项目的 "Content" 目录
			...
			Else
				' 删除项目的 Content 目录下的 CopyPlugin 中的所有文件
...
Sub DeleteFiles(ByVal folderPath)
    ...
    ' 获取插件文件夹路径
    Dim pluginFolderPath : pluginFolderPath = CreateObject("WScript.Shell").CurrentDirectory & "\CopyPlugin\"
